...
export default function IntegrationShowcase() {
  ...
  if (shouldReduceMotion) {
    return (
      <section className="py-16 overflow-hidden bg-black relative" ref={containerRef} id="integrations">
        <div className="container mx-auto text-center mb-10">
          <p className="text-white/40 text-sm font-mono tracking-widest uppercase mb-4">Connect your stack</p>
        ...
        <div className="container mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-3">
          {integrations.slice(0, 8).map((item) => (
            <div key={item.name} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 border border-white/10">
              <img src={item.logo} alt={item.name} className="w-7 h-7 object-contain" loading="lazy" decoding="async" width="28" height="28" />
  ...
  return (
    <section className="py-24 overflow-hidden bg-black relative" ref={containerRef} id="integrations">
      ...
      <div className="flex relative">
        ...
        >
          {[...integrations, ...integrations].map((item, index) => (
            <div key={index} className="flex items-center gap-3 px-6 py-4 rounded-xl glass-panel border border-white/5 hover:border-[#FF5D00]/40 transition-colors min-w-[200px]">
              <img src={item.logo} alt={item.name} className="w-8 h-8 object-contain" loading="lazy" decoding="async" width="32" height="32" />
